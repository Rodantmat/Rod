window.PickCalcConnectors = (() => {
  ...
  async function getDailyHealth(slateDate = '') {
    const result = await requestJson('/health/daily', {
      method: 'GET',
  ...
  async function getLegPacket(row = {}, slateDate = '') {
    ...
    const result = await requestJson('/packet/leg', { method: 'POST', body: payload });
    const body = result.body?.body || result.body || {};
    return {
      ok: result.ok,
      source: 'packet/leg',
  ...
  async function scoreLeg(row = {}, slateDate = '') {
    const payload = buildLegPayload(row, slateDate);
    const result = await requestJson('/score/leg', { method: 'POST', body: payload });
    const body = result.body?.body || result.body || {};
    return {
      ok: result.ok,
      source: 'score/leg',
