window.PickCalcUI = (() => {
  const SYSTEM_VERSION = 'v13.78.05 (OXYGEN-COBALT) • Main-1I Matrix Fill Cache';
  ...
  function buildMatrixSections(row = {}, vault = null, backendHealth = null) {
    ...
    return [
      ...
      renderMatrixSection('Market', [
        ...
        { label: 'Team Implied Runs', value: formatFactorValue(getFactor(packet, 'market', 'implied_runs')), status: getFactor(packet, 'market', 'implied_runs') ? 'ok' : 'missing' },
      ...
      renderMatrixSection('Risk', [
        ...
        { label: 'Candidate Tier', value: formatFactorValue(getFactor(packet, 'risk', 'candidate_tier')), status: getFactor(packet, 'risk', 'candidate_tier') ? 'ok' : 'missing' },
        { label: 'Lineup Context', value: formatFactorValue(getFactor(packet, 'risk', 'lineup_context')), status: getFactor(packet, 'risk', 'lineup_context') ? 'ok' : 'missing' },
