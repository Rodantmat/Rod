window.PickCalcCore = (() => {
  ...
  async function refreshBackendHealth(options = {}) {
    ...
    const attempts = Math.max(1, Number(options.attempts || (auto ? 3 : 1)));
    logEvent('info', auto ? 'Auto Daily Health started' : 'Refresh Daily Health clicked / started');
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
      if (attempt > 1) logEvent('warn', `Daily Health retry ${attempt}/${attempts} started`);
      state.backendHealth = await Connectors.getDailyHealth(state.slateDate);
      UI.renderBackendStatus(state.backendHealth);
      if (state.backendHealth?.ok || attempt === attempts) break;
  ...
  async function runDatabaseWiringPass(options = {}) {
    const auto = !!options.auto;
  ...
  function bindEvents() {
    ...
    if (rerunBackendBtn) rerunBackendBtn.addEventListener('click', async () => {
      ...
      state.miningVault = {};
      await refreshBackendHealth({ auto: false, attempts: 1 });
      await runDatabaseWiringPass({ auto: false });
    });
