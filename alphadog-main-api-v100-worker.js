const WORKER_VERSION = "alphadog-main-api-v100.11 - Main-1N.4 Goblin Triple Tap";
...
async function handleDebugRoutes() {
  return jsonResponse({
    ...
    routes: [
      ...
      "POST /main/score/leg",
      "POST /main/gemini/matrix/prompt",
...
const GEMINI_PRIMARY_TIMEOUT_MS = 30000;
const GEMINI_FALLBACK_TIMEOUT_MS = 30000;
const GEMINI_MAX_ATTEMPTS_PER_MODEL = 3;
const GEMINI_PROMPT_DEFS = [
  {prompt_id:"GEMINI_A_PLAYER_ROLE_RECENT_MATCHUP_V2",prompt_key:"A",title:"Player role, recent form, matchup",factors:[["A01","Player identity confirmed"],["A02","Lineup slot quality"],["A03","Role stability"],["A04","Recent hit trend last 5"],["A05","Recent plate appearance volume"],["A06","Season hit profile"],["A07","Handedness matchup"],["A08","Opposing starter contact allowance"],["A09","Strikeout pressure risk"],["A10","Walk/on-base support"]]},
  {prompt_id:"GEMINI_B_GAME_TEAM_BULLPEN_ENVIRONMENT_V1",prompt_key:"B",title:"Game, team, bullpen, environment context",factors:[["B01","Game context available"],["B02","Team lineup depth around player"],["B03","Team recent run environment"],["B04","Team recent hit volume"],["B05","Opposing starter run prevention"],["B06","Opposing starter baserunner allowance"],["B07","Opposing starter hit allowance"],["B08","Opposing starter HR/contact damage"],["B09","Opponent bullpen fatigue"],["B10","Opponent bullpen recent workload"],["B11","Park run factor"],["B12","Park HR/contact factor"]]},
  {prompt_id:"GEMINI_C_PROP_MARKET_CANDIDATE_GAMESTATE_V1",prompt_key:"C",title:"Prop line, market placeholder, candidate and game state",factors:[["C01","Prop line difficulty"],["C02","Prop side clarity"],["C03","Market availability"],["C04","Game total scoring context"],["C05","Team implied run support"],["C06","Moneyline/game expectation"],["C07","Candidate pool support"],["C08","Candidate reason density"],["C09","Live/final game-state risk"],["C10","Market-source integrity"]]},
  {prompt_id:"GEMINI_D_ADVANCED_PLAYER_FORM_CONTACT_V1",prompt_key:"D",title:"Advanced player form and contact",factors:[["D01","Last 5 hit efficiency"],["D02","Last 3 hit pressure"],["D03","Season batting average stability"],["D04","Season on-base support"],["D05","Season slug/contact damage"],["D06","Hits per game baseline"],["D07","Strikeout drag"],["D08","Recent total-base contact"],["D09","Walk displacement risk"],["D10","Current form vs season baseline"]]},
  {prompt_id:"GEMINI_E_INJURY_NEWS_WEATHER_INTEGRITY_V1",prompt_key:"E",title:"Injury, news, weather and integrity placeholders",factors:[["E01","Player injury/news risk"],["E02","Lineup confirmation safety"],["E03","Late scratch risk"],["E04","Team news disruption"],["E05","Weather availability"],["E06","Weather risk"],["E07","Wind risk"],["E08","Precipitation risk"],["E09","Game-state integrity"],["E10","Final integrity readiness"]]}
];

function sleep(ms){return new Promise(resolve=>setTimeout(resolve,ms));}
function signalFromScore(score){const value=clamp(score,0,100); if(value>=80)return "GREEN"; if(value>=65)return "YELLOW"; return "RED";}
function confidenceFromScore(score){const value=clamp(score,0,100); if(value>=80)return "HIGH"; if(value>=65)return "MEDIUM"; return "LOW";}
function compactForGemini(value,maxChars=7000){let text="";try{text=JSON.stringify(value,null,2);}catch(_err){text=safeText(value);}return text.length>maxChars?text.slice(0,maxChars)+"\n...[TRUNCATED_BY_MAIN_API]":text;}
function summarizeRecentUsage(rows=[]){const arr=Array.isArray(rows)?rows.slice(0,5):[];const ab=arr.reduce((s,r)=>s+Number(r.ab||0),0);const h=arr.reduce((s,r)=>s+Number(r.h||0),0);const bb=arr.reduce((s,r)=>s+Number(r.bb||0),0);const so=arr.reduce((s,r)=>s+Number(r.so||0),0);const tb=arr.reduce((s,r)=>s+Number(r.total_bases||0),0);return{games:arr.length,total_ab:ab,total_hits:h,total_bb:bb,total_so:so,total_bases:tb,avg:ab?Number((h/ab).toFixed(3)):null,last_hits:arr.map(r=>Number(r.h||0)).join(" / "),last_ab:arr.map(r=>Number(r.ab||0)).join(" / ")};}
function summarizeTeamGames(rows=[]){const arr=Array.isArray(rows)?rows.slice(0,5):[];const n=arr.length||0;const sum=k=>arr.reduce((s,r)=>s+Number(r[k]||0),0);return{games:n,avg_runs_for:n?Number((sum("runs_for")/n).toFixed(2)):null,avg_hits_for:n?Number((sum("hits_for")/n).toFixed(2)):null,avg_so:n?Number((sum("strikeouts")/n).toFixed(2)):null,avg_walks:n?Number((sum("walks")/n).toFixed(2)):null};}
function sanitizePacketForGeminiPrompt(def,packet){const mlb=packet?.mlb_api||{};const base={slate_date:packet?.slate_date||null,leg:packet?.leg||null,missing:packet?.missing||[],warnings:packet?.warnings||[]};const key=def.prompt_key;const recent_summary=summarizeRecentUsage(packet?.recent_usage||[]);const team_recent_summary=mlb.player_recent_summary||summarizeTeamGames(mlb.team_recent_games||[]);if(key==="A")return{...base,player:packet?.player||null,lineup:packet?.lineup||null,opposing_starter:packet?.opposing_starter||null,recent_summary,season_profile:packet?.player?{games:packet.player.games,ab:packet.player.ab,hits:packet.player.hits,avg:packet.player.avg,obp:packet.player.obp,slg:packet.player.slg,bats:packet.player.bats,position:packet.player.position}:null};if(key==="B")return{...base,game:packet?.game||null,lineup:packet?.lineup||null,team_recent_summary:summarizeTeamGames(mlb.team_recent_games||[]),opposing_starter:packet?.opposing_starter||null,bullpen:packet?.bullpen||null,opponent_bullpen_recent:Array.isArray(mlb.opponent_bullpen_recent)?mlb.opponent_bullpen_recent.slice(0,3):[],venue:packet?.game?.venue||null,market:packet?.market||null};if(key==="C")return{...base,game:packet?.game||null,market:packet?.market||null,candidate:packet?.candidate||null,related_candidates:packet?.related_candidates||null,matrix_factor_summary:packet?.matrix_factor_summary||null,live_status:mlb.live_status||null};if(key==="D")return{...base,player:packet?.player||null,recent_summary,season_profile:packet?.player?{games:packet.player.games,ab:packet.player.ab,hits:packet.player.hits,avg:packet.player.avg,obp:packet.player.obp,slg:packet.player.slg,strikeouts:packet.player.strikeouts,walks:packet.player.walks}:null,recent_usage:Array.isArray(packet?.recent_usage)?packet.recent_usage.slice(0,5):[]};return{...base,game:packet?.game||null,lineup:packet?.lineup||null,player:packet?.player?{player_name:packet.player.player_name,team_id:packet.player.team_id,role:packet.player.role,position:packet.player.position}:null,weather:mlb.weather||null,live_status:mlb.live_status||null,integrity_placeholders:{player_injury_status:"not_wired_unless_supplied",player_news_flag:"not_wired_unless_supplied",late_scratch_risk:"not_wired_unless_supplied",team_news_flag:"not_wired_unless_supplied"}};}
function buildGeminiPrompt(def,systemData){const factorLines=def.factors.map(([id,name])=>`- ${id}: ${name}`).join("\n");return `You are OXYGEN-COBALT deterministic MLB matrix prompt ${def.prompt_key}.\nReturn JSON only. No markdown. No text outside JSON.\nUse only supplied SYSTEM_DATA. Do not browse. Do not invent odds, injuries, news, weather, or market data.\nDo not make final picks. Do not create final scoring. Matrix fill only.\nReturn every required factor exactly once. Keep factor IDs and names unchanged.\nUse score_0_100 from 0 to 100. Signals: GREEN >=80, YELLOW 65-79, RED <65.\nIf source data is missing, use score_0_100 50, signal YELLOW, confidence LOW, and add factor_id to summary.missing_data.\nBackend will recompute summary counts; still return summary.\n\nPROMPT_ID: ${def.prompt_id}\nPROMPT_TITLE: ${def.title}\n\nREQUIRED FACTORS:\n${factorLines}\n\nOUTPUT SCHEMA:\n{"ok":true,"prompt_id":"${def.prompt_id}","prompt_key":"${def.prompt_key}","leg_identity":{},"factors":[{"factor_id":"${def.factors[0][0]}","factor_name":"${def.factors[0][1]}","source_type":"SYSTEM_DATA|MISSING_DATA|SYSTEM_PLACEHOLDER","raw_data":"brief sourced value","score_0_100":0,"signal":"GREEN|YELLOW|RED","confidence":"HIGH|MEDIUM|LOW","note":"brief note"}],"summary":{"average_score":0,"green_count":0,"yellow_count":0,"red_count":0,"missing_data":[],"warnings":[],"ready_for_system_json":true}}\n\nSYSTEM_DATA:\n${compactForGemini(systemData)}\n`;}
function extractGeminiText(body){const candidates=Array.isArray(body?.candidates)?body.candidates:[];const parts=candidates[0]?.content?.parts||[];return parts.map(p=>p.text||"").join("\n").trim();}
function parseGeminiJson(text){const raw=safeText(text).trim().replace(/^```json\s*/i,"").replace(/^```\s*/i,"").replace(/```$/i,"").trim();if(!raw)throw new Error("zero_payload_empty_text");try{return JSON.parse(raw);}catch(_err){}const start=raw.indexOf("{");const end=raw.lastIndexOf("}");if(start>=0&&end>start)return JSON.parse(raw.slice(start,end+1));throw new Error("malformed_json");}
function normalizeGeminiOutput(def,parsed,meta={}){if(!parsed||typeof parsed!=="object"||Object.keys(parsed).length===0)return{ok:false,status:"zero_payload",error:"Gemini returned empty JSON object"};if(parsed.prompt_id&&parsed.prompt_id!==def.prompt_id)return{ok:false,status:"malformed_json",error:`Prompt ID mismatch: ${parsed.prompt_id}`};const factorsIn=Array.isArray(parsed.factors)?parsed.factors:[];if(!factorsIn.length)return{ok:false,status:"zero_payload",error:"Gemini returned no factors"};const byId=new Map(factorsIn.map(f=>[safeText(f.factor_id).toUpperCase(),f]));const missingData=[];const warnings=Array.isArray(parsed.summary?.warnings)?parsed.summary.warnings.slice():[];const factors=def.factors.map(([factorId,factorName])=>{const raw=byId.get(factorId);if(!raw){missingData.push(factorId);warnings.push(`${factorId} missing from Gemini response; backend inserted missing placeholder`);return{factor_id:factorId,factor_name:factorName,source_type:"MISSING_DATA",raw_data:"MISSING_FROM_GEMINI_RESPONSE",score_0_100:50,signal:"YELLOW",confidence:"LOW",note:"Gemini did not return this required factor; backend preserved matrix shape."};}const score=clamp(raw.score_0_100,0,100);const sig=safeText(raw.signal).toUpperCase();const conf=safeText(raw.confidence).toUpperCase();return{factor_id:factorId,factor_name:factorName,source_type:safeText(raw.source_type||"SYSTEM_DATA"),raw_data:safeText(raw.raw_data||""),score_0_100:score,signal:["GREEN","YELLOW","RED"].includes(sig)?sig:signalFromScore(score),confidence:["HIGH","MEDIUM","LOW"].includes(conf)?conf:confidenceFromScore(score),note:safeText(raw.note||"")};});const avg=factors.length?Number((factors.reduce((sum,f)=>sum+Number(f.score_0_100||0),0)/factors.length).toFixed(1)):0;const green=factors.filter(f=>f.signal==="GREEN").length;const yellow=factors.filter(f=>f.signal==="YELLOW").length;const red=factors.filter(f=>f.signal==="RED").length;const parsedMissing=Array.isArray(parsed.summary?.missing_data)?parsed.summary.missing_data.map(String):[];return{ok:true,status:"ok",prompt_id:def.prompt_id,prompt_key:def.prompt_key,title:def.title,model:meta.model||GEMINI_PRIMARY_MODEL,model_role:meta.model_role||"primary",attempts:meta.attempts||1,duration_ms:meta.duration_ms||null,factors,summary:{average_score:avg,green_count:green,yellow_count:yellow,red_count:red,missing_data:Array.from(new Set([...parsedMissing,...missingData])),warnings:Array.from(new Set(warnings.map(String))),ready_for_system_json:true,backend_recomputed:true},raw_prompt_summary:parsed.summary||null};}
function isRetryableGeminiStatus(status,errorText=""){const text=safeText(errorText).toLowerCase();if(status===404||text.includes("not found")||text.includes("not supported for generatecontent"))return false;return status===429||status===503||text.includes("timeout")||text.includes("zero_payload")||text.includes("empty")||text.includes("malformed_json")||text.includes("abort")||text.includes("cancel")||text.includes("fetch")||text.includes("busy")||text.includes("overload")||text.includes("high demand");}
function friendlyGeminiError(errorText="",status=0){const text=safeText(errorText).toLowerCase();if(text.includes("missing_gemini_api_key")||text.includes("api key"))return{code:"GEMINI_KEY_MISSING",message:"Gemini API key is missing or not available on the Worker.",is_retryable:false,suggested_wait_ms:0};if(status===404||text.includes("not found")||text.includes("not supported for generatecontent"))return{code:"MODEL_UNAVAILABLE_404",message:"Gemini model is unavailable or not supported by this API key. Check the configured model name.",is_retryable:false,suggested_wait_ms:0};if(status===503||text.includes("high demand")||text.includes("busy")||text.includes("overload"))return{code:"MODEL_OVERLOAD_503",message:"Gemini is busy now, try again in a few minutes.",is_retryable:true,suggested_wait_ms:180000};if(status===429||text.includes("quota")||text.includes("rate"))return{code:"MODEL_RATE_LIMIT_429",message:"Gemini rate limit reached, try again in a few minutes.",is_retryable:true,suggested_wait_ms:300000};if(text.includes("timeout")||text.includes("cancelled")||text.includes("abort"))return{code:"MODEL_TIMEOUT",message:"Gemini timed out, try again in a few minutes.",is_retryable:true,suggested_wait_ms:180000};if(text.includes("malformed")||text.includes("json"))return{code:"MALFORMED_JSON",message:"Gemini returned malformed JSON. Try again.",is_retryable:true,suggested_wait_ms:60000};if(text.includes("zero_payload")||text.includes("empty"))return{code:"ZERO_PAYLOAD",message:"Gemini returned an empty response. Try again.",is_retryable:true,suggested_wait_ms:60000};return{code:"GEMINI_FAILED",message:safeText(errorText)||"Gemini failed. Try again in a few minutes.",is_retryable:true,suggested_wait_ms:180000};}
async function callGeminiOnce(env,promptText,modelName,timeoutMs){const key=safeText(env.GEMINI_API_KEY).trim();if(!key)throw new Error("missing_GEMINI_API_KEY_secret");const model=safeText(modelName||GEMINI_PRIMARY_MODEL).trim()||GEMINI_PRIMARY_MODEL;const controller=new AbortController();const timer=setTimeout(()=>controller.abort("gemini_timeout"),timeoutMs);const started=Date.now();try{const url=`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${encodeURIComponent(key)}`;const res=await fetch(url,{method:"POST",headers:{"content-type":"application/json"},signal:controller.signal,body:JSON.stringify({contents:[{role:"user",parts:[{text:promptText}]}],generationConfig:{temperature:0.1,topP:0.95,maxOutputTokens:1200,responseMimeType:"application/json"}})});const textBody=await res.text();let body=null;try{body=textBody?JSON.parse(textBody):null;}catch(_err){body={raw:textBody};}if(!res.ok){const err=new Error(body?.error?.message||`Gemini HTTP ${res.status}`);err.http_status=res.status;err.body=body;err.model=model;throw err;}const outputText=extractGeminiText(body);if(!outputText.trim()){const err=new Error("zero_payload_empty_candidates");err.http_status=res.status;err.body=body;err.model=model;throw err;}return{ok:true,model,http_status:res.status,duration_ms:Date.now()-started,text:outputText,body};}catch(err){const text=safeText(err?.message||err).toLowerCase();const isAbort=err?.name==="AbortError"||text.includes("abort")||text.includes("cancel")||text.includes("timeout");const out=new Error(isAbort?"timeout_or_cancelled":(err?.message||String(err)));out.http_status=err?.http_status||0;out.body=err?.body||null;out.duration_ms=Date.now()-started;out.cancelled=isAbort;out.model=err?.model||model;throw out;}finally{clearTimeout(timer);}}
async function runOneGeminiPrompt(env,def,systemData){const promptText=buildGeminiPrompt(def,systemData);const attempts=[];const overallStart=Date.now();let lastStatus="failed";let lastError="unknown_error";let lastHttp=0;let lastModel=GEMINI_PRIMARY_MODEL;let lastRetryable=true;const modelPlan=[{model:GEMINI_PRIMARY_MODEL,role:"primary",timeout:GEMINI_PRIMARY_TIMEOUT_MS},{model:GEMINI_FALLBACK_MODEL,role:"fallback",timeout:GEMINI_FALLBACK_TIMEOUT_MS}];for(const modelEntry of modelPlan){lastModel=modelEntry.model;let modelShouldFallback=false;for(let attempt=1;attempt<=GEMINI_MAX_ATTEMPTS_PER_MODEL;attempt++){try{const res=await callGeminiOnce(env,promptText,modelEntry.model,modelEntry.timeout);let parsed;try{parsed=parseGeminiJson(res.text);}catch(parseErr){const err=new Error(parseErr?.message||"malformed_json");err.http_status=res.http_status;err.duration_ms=res.duration_ms;err.model=res.model;throw err;}const normalized=normalizeGeminiOutput(def,parsed,{attempts:attempt,duration_ms:Date.now()-overallStart,model:res.model,model_role:modelEntry.role});const retryable=normalized.ok?false:isRetryableGeminiStatus(res.http_status,normalized.error||normalized.status);attempts.push({model:res.model,model_role:modelEntry.role,attempt,status:normalized.ok?"ok":(retryable&&attempt<GEMINI_MAX_ATTEMPTS_PER_MODEL?"retrying":normalized.status),http_status:res.http_status,duration_ms:res.duration_ms,error:normalized.error||null});if(normalized.ok)return{...normalized,model:res.model,model_role:modelEntry.role,attempts_log:attempts};lastStatus=normalized.status||"failed";lastError=normalized.error||lastStatus;lastHttp=res.http_status||0;lastRetryable=retryable;if(!retryable)break;if(attempt<GEMINI_MAX_ATTEMPTS_PER_MODEL)await sleep(attempt===1?1000:2000);}catch(err){const msg=safeText(err?.message||err);const lower=msg.toLowerCase();const status=err?.cancelled?"cancelled":(lower.includes("timeout")?"timeout":(lower.includes("zero_payload")?"zero_payload":(lower.includes("malformed_json")?"malformed_json":"failed")));const http=err?.http_status||0;const retryable=isRetryableGeminiStatus(http,msg);attempts.push({model:err?.model||modelEntry.model,model_role:modelEntry.role,attempt,status:retryable&&attempt<GEMINI_MAX_ATTEMPTS_PER_MODEL?"retrying":status,http_status:http,duration_ms:err?.duration_ms||null,error:msg});lastStatus=status;lastError=msg;lastHttp=http;lastRetryable=retryable;if(!retryable)break;if(attempt<GEMINI_MAX_ATTEMPTS_PER_MODEL)await sleep(attempt===1?1000:2000);}}if(modelEntry.role==="primary"&&lastRetryable){attempts.push({model:GEMINI_FALLBACK_MODEL,model_role:"fallback",attempt:0,status:"fallback_started",http_status:0,duration_ms:null,error:`Primary model exhausted after ${GEMINI_MAX_ATTEMPTS_PER_MODEL} attempt(s): ${lastError}`});modelShouldFallback=true;}if(modelShouldFallback)continue;break;}const ctx=friendlyGeminiError(lastError,lastHttp);return{ok:false,status:lastStatus,prompt_id:def.prompt_id,prompt_key:def.prompt_key,title:def.title,model:lastModel,attempts:attempts.filter(a=>a.attempt>0).length,duration_ms:Date.now()-overallStart,factors:[],summary:{average_score:0,green_count:0,yellow_count:0,red_count:0,missing_data:def.factors.map(f=>f[0]),warnings:[ctx.message],ready_for_system_json:false,backend_recomputed:true},attempts_log:attempts,error:ctx.message,error_context:ctx};}
function summarizeGeminiPrompts(results=[]){const allFactors=results.flatMap(r=>Array.isArray(r.factors)?r.factors:[]);const total=allFactors.length;const ok_prompts=results.filter(r=>r.ok).length;const failed_prompts=results.filter(r=>!r.ok).length;const avg=total?Number((allFactors.reduce((sum,f)=>sum+Number(f.score_0_100||0),0)/total).toFixed(1)):0;return{total_prompts:results.length,ok_prompts,failed_prompts,partial_success:ok_prompts>0&&failed_prompts>0,total_factors:total,green_count:allFactors.filter(f=>f.signal==="GREEN").length,yellow_count:allFactors.filter(f=>f.signal==="YELLOW").length,red_count:allFactors.filter(f=>f.signal==="RED").length,average_factor_score:avg,final_scoring_status:"not_final_matrix_fill_only"};}
function geminiPolicy(route="/main/gemini/matrix/prompt"){return{orchestration:"frontend_sequential_one_prompt_per_worker_request",primary_model:GEMINI_PRIMARY_MODEL,fallback_model:GEMINI_FALLBACK_MODEL,primary_timeout_ms:GEMINI_PRIMARY_TIMEOUT_MS,fallback_timeout_ms:GEMINI_FALLBACK_TIMEOUT_MS,max_attempts_per_model:GEMINI_MAX_ATTEMPTS_PER_MODEL,retry_conditions:["503","429","timeout","cancelled","zero_payload","malformed_json","network_fetch_failure"],fallback_after_primary_failure:true,route,cache:"deferred_first_build_no_gemini_cache_writes"};}
async function handleGeminiMatrixPrompt(request,env){const url=new URL(request.url);const payload=await readJson(request);const promptKey=safeText(url.searchParams.get("prompt_key")||payload.prompt_key||payload.prompt_id||"").toUpperCase();const def=GEMINI_PROMPT_DEFS.find(d=>d.prompt_key===promptKey||d.prompt_id===promptKey);if(!def)return jsonResponse({ok:false,version:WORKER_VERSION,worker:"alphadog-main-api-v100",mode:"gemini_single_prompt_matrix_fill_only_no_final_scoring",route:"/main/gemini/matrix/prompt",status:"failed",error:"missing_or_invalid_prompt_key",error_context:{code:"INVALID_PROMPT_KEY",message:"Missing or invalid Gemini prompt key. Use A, B, C, D, or E.",is_retryable:false,suggested_wait_ms:0},allowed_prompt_keys:GEMINI_PROMPT_DEFS.map(d=>d.prompt_key)},400);const packet=await buildLegPacket(payload,env,{writeCache:false});const systemData=sanitizePacketForGeminiPrompt(def,packet);const startedAt=new Date().toISOString();const prompt=await runOneGeminiPrompt(env,def,systemData);const summary=summarizeGeminiPrompts([prompt]);const status=prompt.ok?"ok":"failed";return jsonResponse({ok:!!prompt.ok,version:WORKER_VERSION,worker:"alphadog-main-api-v100",mode:"gemini_single_prompt_matrix_fill_only_no_final_scoring",route:"/main/gemini/matrix/prompt",slate_date:packet.slate_date,leg:packet.leg,prompt_key:def.prompt_key,prompt_id:def.prompt_id,policy:geminiPolicy(),status,summary,prompt,error:prompt.ok?null:prompt.error,error_context:prompt.error_context||null,packet_digest:{packet_ok:packet.ok,game_id:packet.game_id,player_found:!!packet.player,lineup_found:!!packet.lineup,starter_found:!!packet.opposing_starter,candidate_found:!!packet.candidate,matrix_factor_summary:packet.matrix_factor_summary,missing:packet.missing||[],warnings:packet.warnings||[]},started_at:startedAt,finished_at:new Date().toISOString(),note:"Single Gemini prompt matrix fill only. Final scoring remains placeholder and will be sharpened later."});}
async function handleGeminiMatrixLeg(request,env){const payload=await readJson(request);const packet=await buildLegPacket(payload,env,{writeCache:false});const startedAt=new Date().toISOString();const prompts=[];for(const def of GEMINI_PROMPT_DEFS){const systemData=sanitizePacketForGeminiPrompt(def,packet);prompts.push(await runOneGeminiPrompt(env,def,systemData));}const summary=summarizeGeminiPrompts(prompts);const ok=summary.ok_prompts>0;const firstError=prompts.find(p=>!p.ok)?.error_context||null;return jsonResponse({ok,version:WORKER_VERSION,worker:"alphadog-main-api-v100",mode:"gemini_a_e_matrix_fill_only_no_final_scoring_sequential_legacy",route:"/main/gemini/matrix/leg",slate_date:packet.slate_date,leg:packet.leg,policy:geminiPolicy("/main/gemini/matrix/leg"),status:summary.failed_prompts===0?"ok":(summary.ok_prompts>0?"partial_success":"failed"),summary,prompts,error_context:firstError,packet_digest:{packet_ok:packet.ok,game_id:packet.game_id,player_found:!!packet.player,lineup_found:!!packet.lineup,starter_found:!!packet.opposing_starter,candidate_found:!!packet.candidate,matrix_factor_summary:packet.matrix_factor_summary,missing:packet.missing||[],warnings:packet.warnings||[]},started_at:startedAt,finished_at:new Date().toISOString(),note:"Legacy full A-E route now runs sequentially. Frontend should prefer /main/gemini/matrix/prompt."});}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === "POST" && path === "/main/gemini/matrix/prompt") return await handleGeminiMatrixPrompt(request, env);
      if (request.method === "POST" && path === "/main/gemini/matrix/leg") return await handleGeminiMatrixLeg(request, env);
      return jsonResponse({
        ok: false,
        error: "Not found",
        worker: "alphadog-main-api-v100",
        available_routes: ["GET /main/debug/routes", "GET /main/health?slate_date=YYYY-MM-DD", "POST /main/packet/leg", "POST /main/score/leg", "POST /main/gemini/matrix/prompt",
      "POST /main/gemini/matrix/prompt", "POST /main/gemini/matrix/leg"]
      }, 404);
    } catch (err) {
