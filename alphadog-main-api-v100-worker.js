...
function compactForGemini(value,maxChars=16000){let text="";try{text=JSON.stringify(value,null,2);}catch(_err){text=safeText(value);}return text.length>maxChars?text.slice(0,maxChars)+"\n...[TRUNCATED_BY_MAIN_API]":text;}
function sanitizePacketForGemini(packet){const mlb=packet?.mlb_api||{};return{slate_date:packet?.slate_date||null,leg:packet?.leg||null,game:packet?.game||null,market:packet?.market||null,player:packet?.player||null,lineup:packet?.lineup||null,opposing_starter:packet?.opposing_starter||null,bullpen:packet?.bullpen||null,recent_usage:Array.isArray(packet?.recent_usage)?packet.recent_usage.slice(0,5):[],candidate:packet?.candidate||null,related_candidates:packet?.related_candidates||null,db_inventory:packet?.db_inventory||null,matrix_factor_summary:packet?.matrix_factor_summary||null,matrix_factors:packet?.matrix_factors||null,mlb_api:{ok:!!mlb.ok,game_pk:mlb.game_pk||null,live_status:mlb.live_status||null,weather:mlb.weather||null,officials:mlb.officials||null,player_recent_summary:mlb.player_recent_summary||null,team_recent_summary:mlb.team_recent_summary||null,opponent_bullpen_recent_summary:mlb.opponent_bullpen_recent_summary||null,opponent_starter_recent_summary:mlb.opponent_starter_recent_summary||null,current_player_boxscore:mlb.current_player_boxscore||null,team_recent_games:Array.isArray(mlb.team_recent_games)?mlb.team_recent_games.slice(0,5):[],opponent_bullpen_recent:Array.isArray(mlb.opponent_bullpen_recent)?mlb.opponent_bullpen_recent.slice(0,3):[],warnings:mlb.warnings||[]},missing:packet?.missing||[],warnings:packet?.warnings||[]};}
...
function isRetryableGeminiStatus(status,errorText=""){const text=safeText(errorText).toLowerCase();return status===429||status===503||text.includes("timeout")||text.includes("zero_payload")||text.includes("empty")||text.includes("malformed_json")||text.includes("abort")||text.includes("cancel")||text.includes("fetch");}
async function callGeminiOnce(env,promptText,modelName){const key=safeText(env.GEMINI_API_KEY).trim();if(!key)throw new Error("missing_GEMINI_API_KEY_secret");const model=safeText(modelName||GEMINI_PRIMARY_MODEL).trim()||GEMINI_PRIMARY_MODEL;const controller=new AbortController();const timer=setTimeout(()=>controller.abort("gemini_timeout"),GEMINI_TIMEOUT_MS);const started=Date.now();try{const url=`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${encodeURIComponent(key)}`;const res=await fetch(url,{method:"POST",headers:{"content-type":"application/json"},signal:controller.signal,body:JSON.stringify({contents:[{role:"user",parts:[{text:promptText}]}],generationConfig:{temperature:0,topP:1,topK:1,responseMimeType:"application/json"}})});const textBody=await res.text();let body=null;try{body=textBody?JSON.parse(textBody):null;}catch(_err){body={raw:textBody};}if(!res.ok){const err=new Error(body?.error?.message||`Gemini HTTP ${res.status}`);err.http_status=res.status;err.body=body;err.model=model;throw err;}const outputText=extractGeminiText(body);if(!outputText.trim()){const err=new Error("zero_payload_empty_candidates");err.http_status=res.status;err.body=body;err.model=model;throw err;}return{ok:true,model,http_status:res.status,duration_ms:Date.now()-started,text:outputText,body};}catch(err){const text=safeText(err?.message||err).toLowerCase();const isAbort=err?.name==="AbortError"||text.includes("abort")||text.includes("cancel")||text.includes("timeout");const out=new Error(isAbort?"timeout_or_cancelled":(err?.message||String(err)));out.http_status=err?.http_status||0;out.body=err?.body||null;out.duration_ms=Date.now()-started;out.cancelled=isAbort;out.model=err?.model||model;throw out;}finally{clearTimeout(timer);}}
async function runOneGeminiPrompt(env,def,systemData){const promptText=buildGeminiPrompt(def,systemData);const attempts=[];const overallStart=Date.now();let lastStatus="failed";let lastError="unknown_error";let lastModel=GEMINI_PRIMARY_MODEL;const modelPlan=[{model:GEMINI_PRIMARY_MODEL,role:"primary"},{model:GEMINI_FALLBACK_MODEL,role:"fallback"}];for(const modelEntry of modelPlan){for(let attempt=1;attempt<=GEMINI_MAX_ATTEMPTS;attempt++){lastModel=modelEntry.model;try{const res=await callGeminiOnce(env,promptText,modelEntry.model);let parsed;try{parsed=parseGeminiJson(res.text);}catch(parseErr){const err=new Error(parseErr?.message||"malformed_json");err.http_status=res.http_status;err.duration_ms=res.duration_ms;err.model=res.model;throw err;}const normalized=normalizeGeminiOutput(def,parsed,{attempts:attempt,duration_ms:Date.now()-overallStart,model:res.model,model_role:modelEntry.role});attempts.push({model:res.model,model_role:modelEntry.role,attempt,status:normalized.ok?"ok":normalized.status,http_status:res.http_status,duration_ms:res.duration_ms,error:normalized.error||null});if(normalized.ok)return{...normalized,model:res.model,model_role:modelEntry.role,attempts_log:attempts};lastStatus=normalized.status||"failed";lastError=normalized.error||lastStatus;if(!isRetryableGeminiStatus(res.http_status,normalized.error)||attempt===GEMINI_MAX_ATTEMPTS)break;}catch(err){const msg=safeText(err?.message||err);const lower=msg.toLowerCase();const status=err?.cancelled?"cancelled":(lower.includes("timeout")?"timeout":(lower.includes("zero_payload")?"zero_payload":(lower.includes("malformed_json")?"malformed_json":"failed")));const retryable=isRetryableGeminiStatus(err?.http_status||0,msg);attempts.push({model:err?.model||modelEntry.model,model_role:modelEntry.role,attempt,status:attempt<GEMINI_MAX_ATTEMPTS&&retryable?"retrying":status,http_status:err?.http_status||0,duration_ms:err?.duration_ms||null,error:msg});lastStatus=status;lastError=msg;if(!retryable||attempt===GEMINI_MAX_ATTEMPTS)break;}await sleep(GEMINI_RETRY_DELAYS_MS[Math.min(attempt-1,GEMINI_RETRY_DELAYS_MS.length-1)]||1000);}if(modelEntry.role==="primary"&&isRetryableGeminiStatus(0,lastError)){attempts.push({model:GEMINI_FALLBACK_MODEL,model_role:"fallback",attempt:0,status:"fallback_started",http_status:0,duration_ms:null,error:`Primary model exhausted: ${lastError}`});continue;}break;}return{ok:false,status:lastStatus,prompt_id:def.prompt_id,prompt_key:def.prompt_key,title:def.title,model:lastModel,attempts:attempts.filter(a=>a.attempt>0).length,duration_ms:Date.now()-overallStart,factors:[],summary:{average_score:0,green_count:0,yellow_count:0,red_count:0,missing_data:def.factors.map(f=>f[0]),warnings:[lastError],ready_for_system_json:false,backend_recomputed:true},attempts_log:attempts,error:lastError};}
...
async function handleGeminiMatrixLeg(request,env){const payload=await readJson(request);const packet=await buildLegPacket(payload,env,{writeCache:false});const systemData=sanitizePacketForGemini(packet);const startedAt=new Date().toISOString();const settled=await Promise.allSettled(GEMINI_PROMPT_DEFS.map(def=>runOneGeminiPrompt(env,def,systemData)));const prompts=settled.map((item,index)=>{if(item.status==="fulfilled")return item.value;const def=GEMINI_PROMPT_DEFS[index];return{ok:false,status:"failed",prompt_id:def.prompt_id,prompt_key:def.prompt_key,title:def.title,factors:[],summary:{average_score:0,green_count:0,yellow_count:0,red_count:0,missing_data:def.factors.map(f=>f[0]),warnings:[item.reason?.message||String(item.reason)],ready_for_system_json:false,backend_recomputed:true},attempts_log:[{attempt:1,status:"failed",error:item.reason?.message||String(item.reason)}],error:item.reason?.message||String(item.reason)};});const summary=summarizeGeminiPrompts(prompts);const ok=summary.ok_prompts>0;return jsonResponse({ok,version:WORKER_VERSION,worker:"alphadog-main-api-v100",mode:"gemini_a_e_matrix_fill_only_no_final_scoring",route:"/main/gemini/matrix/leg",slate_date:packet.slate_date,leg:packet.leg,policy:{orchestration:"Promise.allSettled",primary_model:GEMINI_PRIMARY_MODEL,fallback_model:GEMINI_FALLBACK_MODEL,timeout_ms_per_prompt:GEMINI_TIMEOUT_MS,max_attempts_per_model:GEMINI_MAX_ATTEMPTS,retry_delays_ms:GEMINI_RETRY_DELAYS_MS,retry_conditions:["503","429","timeout","cancelled","zero_payload","malformed_json","network_fetch_failure"],fallback_after_primary_retry_exhaustion:true,cache:"deferred_first_build_no_gemini_cache_writes"},status:summary.failed_prompts===0?"ok":(summary.ok_prompts>0?"partial_success":"failed"),summary,prompts,packet_digest:{packet_ok:packet.ok,game_id:packet.game_id,player_found:!!packet.player,lineup_found:!!packet.lineup,starter_found:!!packet.opposing_starter,candidate_found:!!packet.candidate,matrix_factor_summary:packet.matrix_factor_summary,missing:packet.missing||[],warnings:packet.warnings||[]},started_at:startedAt,finished_at:new Date().toISOString(),note:"Gemini A-E matrix fill only. Final scoring remains placeholder and will be sharpened later."});}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      return jsonResponse({
        ...
        error: "Not found",
        worker: "alphadog-main-api-v100",
        available_routes: ["GET /main/debug/routes", "GET /main/health?slate_date=YYYY-MM-DD", "POST /main/packet/leg", "POST /main/score/leg", "POST /main/gemini/matrix/leg"]
      }, 404);
    } catch (err) {
