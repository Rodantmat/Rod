const WORKER_VERSION = "alphadog-main-api-v100.10 - Main-1N.3 Goblin Throttle Valve";
...
const GEMINI_PRIMARY_MODEL = "gemini-1.5-pro-latest";
...
const GEMINI_MAX_ATTEMPTS_PER_MODEL = 1;
...
function isRetryableGeminiStatus(status,errorText=""){const text=safeText(errorText).toLowerCase();return status===429||status===503||text.includes("timeout")||text.includes("zero_payload")||text.includes("empty")||text.includes("malformed_json")||text.includes("abort")||text.includes("cancel")||text.includes("fetch")||text.includes("busy")||text.includes("overload");}
...
async function runOneGeminiPrompt(env,def,systemData){const promptText=buildGeminiPrompt(def,systemData);const attempts=[];const overallStart=Date.now();let lastStatus="failed";let lastError="unknown_error";let lastHttp=0;let lastModel=GEMINI_PRIMARY_MODEL;const modelPlan=[{model:GEMINI_PRIMARY_MODEL,role:"primary",timeout:GEMINI_PRIMARY_TIMEOUT_MS},{model:GEMINI_FALLBACK_MODEL,role:"fallback",timeout:GEMINI_FALLBACK_TIMEOUT_MS}];for(const modelEntry of modelPlan){lastModel=modelEntry.model;try{const res=await callGeminiOnce(env,promptText,modelEntry.model,modelEntry.timeout);let parsed;try{parsed=parseGeminiJson(res.text);}catch(parseErr){const err=new Error(parseErr?.message||"malformed_json");err.http_status=res.http_status;err.duration_ms=res.duration_ms;err.model=res.model;throw err;}const normalized=normalizeGeminiOutput(def,parsed,{attempts:1,duration_ms:Date.now()-overallStart,model:res.model,model_role:modelEntry.role});attempts.push({model:res.model,model_role:modelEntry.role,attempt:1,status:normalized.ok?"ok":normalized.status,http_status:res.http_status,duration_ms:res.duration_ms,error:normalized.error||null});if(normalized.ok)return{...normalized,model:res.model,model_role:modelEntry.role,attempts_log:attempts};lastStatus=normalized.status||"failed";lastError=normalized.error||lastStatus;lastHttp=res.http_status||0;if(!isRetryableGeminiStatus(lastHttp,lastError))break;}catch(err){const msg=safeText(err?.message||err);const lower=msg.toLowerCase();const status=err?.cancelled?"cancelled":(lower.includes("timeout")?"timeout":(lower.includes("zero_payload")?"zero_payload":(lower.includes("malformed_json")?"malformed_json":"failed")));attempts.push({model:err?.model||modelEntry.model,model_role:modelEntry.role,attempt:1,status,http_status:err?.http_status||0,duration_ms:err?.duration_ms||null,error:msg});lastStatus=status;lastError=msg;lastHttp=err?.http_status||0;if(!isRetryableGeminiStatus(lastHttp,msg))break;}if(modelEntry.role==="primary"&&isRetryableGeminiStatus(lastHttp,lastError)){attempts.push({model:GEMINI_FALLBACK_MODEL,model_role:"fallback",attempt:0,status:"fallback_started",http_status:0,duration_ms:null,error:`Primary model failed: ${lastError}`});continue;}break;}const ctx=friendlyGeminiError(lastError,lastHttp);return{ok:false,status:lastStatus,prompt_id:def.prompt_id,prompt_key:def.prompt_key,title:def.title,model:lastModel,attempts:attempts.filter(a=>a.attempt>0).length,duration_ms:Date.now()-overallStart,factors:[],summary:{average_score:0,green_count:0,yellow_count:0,red_count:0,missing_data:def.factors.map(f=>f[0]),warnings:[ctx.message],ready_for_system_json:false,backend_recomputed:true},attempts_log:attempts,error:ctx.message,error_context:ctx};}
