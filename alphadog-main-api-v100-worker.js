const WORKER_VERSION = "alphadog-main-api-v100.7 - Main-1N Gemini Goblin Matrix";
...
async function callGeminiOnce(env,promptText){const key=safeText(env.GEMINI_API_KEY).trim();if(!key)throw new Error("missing_GEMINI_API_KEY_secret");const controller=new AbortController();const timer=setTimeout(()=>controller.abort("gemini_timeout"),GEMINI_TIMEOUT_MS);const started=Date.now();try{const url=`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${encodeURIComponent(key)}`;const res=await fetch(url,{method:"POST",headers:{"content-type":"application/json"},signal:controller.signal,body:JSON.stringify({contents:[{role:"user",parts:[{text:promptText}]}],generationConfig:{temperature:0,topP:1,topK:1,responseMimeType:"application/json"}})});const textBody=await res.text();let body=null;try{body=textBody?JSON.parse(textBody):null;}catch(_err){body={raw:textBody};}if(!res.ok){const err=new Error(body?.error?.message||`Gemini HTTP ${res.status}`);err.http_status=res.status;err.body=body;throw err;}const outputText=extractGeminiText(body);if(!outputText.trim()){const err=new Error("zero_payload_empty_candidates");err.http_status=res.status;err.body=body;throw err;}return{ok:true,http_status:res.status,duration_ms:Date.now()-started,text:outputText,body};}catch(err){const text=safeText(err?.message||err).toLowerCase();const isAbort=err?.name==="AbortError"||text.includes("abort")||text.includes("cancel")||text.includes("timeout");const out=new Error(isAbort?"timeout_or_cancelled":(err?.message||String(err)));out.http_status=err?.http_status||0;out.body=err?.body||null;out.duration_ms=Date.now()-started;out.cancelled=isAbort;throw out;}finally{clearTimeout(timer);}}
async function runOneGeminiPrompt(env,def,systemData){const promptText=buildGeminiPrompt(def,systemData);const attempts=[];const overallStart=Date.now();for(let attempt=1;attempt<=GEMINI_MAX_ATTEMPTS;attempt++){try{const res=await callGeminiOnce(env,promptText);let parsed;try{parsed=parseGeminiJson(res.text);}catch(parseErr){const err=new Error(parseErr?.message||"malformed_json");err.http_status=res.http_status;err.duration_ms=res.duration_ms;throw err;}const normalized=normalizeGeminiOutput(def,parsed,{attempts:attempt,duration_ms:Date.now()-overallStart});attempts.push({attempt,status:normalized.ok?"ok":normalized.status,http_status:res.http_status,duration_ms:res.duration_ms,error:normalized.error||null});if(normalized.ok)return{...normalized,attempts_log:attempts};if(!isRetryableGeminiStatus(res.http_status,normalized.error)||attempt===GEMINI_MAX_ATTEMPTS)return{...normalized,attempts:attempt,duration_ms:Date.now()-overallStart,attempts_log:attempts};}catch(err){const msg=safeText(err?.message||err);const status=err?.cancelled?"cancelled":(msg.includes("timeout")?"timeout":(msg.includes("zero_payload")?"zero_payload":(msg.includes("malformed_json")?"malformed_json":"failed")));attempts.push({attempt,status:attempt<GEMINI_MAX_ATTEMPTS&&isRetryableGeminiStatus(err?.http_status||0,msg)?"retrying":status,http_status:err?.http_status||0,duration_ms:err?.duration_ms||null,error:msg});if(!isRetryableGeminiStatus(err?.http_status||0,msg)||attempt===GEMINI_MAX_ATTEMPTS)return{ok:false,status,prompt_id:def.prompt_id,prompt_key:def.prompt_key,title:def.title,model:GEMINI_MODEL,attempts:attempt,duration_ms:Date.now()-overallStart,factors:[],summary:{average_score:0,green_count:0,yellow_count:0,red_count:0,missing_data:def.factors.map(f=>f[0]),warnings:[msg],ready_for_system_json:false,backend_recomputed:true},attempts_log:attempts,error:msg};}await sleep(GEMINI_RETRY_DELAYS_MS[Math.min(attempt-1,GEMINI_RETRY_DELAYS_MS.length-1)]||1000);}}
