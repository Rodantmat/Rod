...
(() => {
  ...
  function renderRunSummary(rows, auditRows = []) {
    ...
    const sportCount = accepted ? `MLB [${accepted}]` : '';
    if (!accepted && !rejected) {
      mount.innerHTML = '';
      return;
    }
    const badges = [];
    if (sportCount) badges.push(`<div class="feed-sport-badge">${escapeHtml(sportCount)}</div>`);
    if (rejected) badges.push(`<div class="feed-sport-badge">Rejected [${escapeHtml(String(rejected))}]</div>`);
    mount.innerHTML = `<div class="status-panel feed-status-inline-panel"><div class="feed-badge-row">${badges.join('')}</div></div>`;
  }
  ...
  function deriveRunStatus(result = {}, rows = [], vaultCollection = {}) {
    ...
    if (Object.values(vaultCollection || {}).some((vault) => vault?.insufficientEvidence === true || vault?.proofFlags?.insufficientEvidence === true)) return 'INSUFFICIENT_EVIDENCE';
  ...
  function branchDisplayScore(branch = {}) {
    const parsedValues = Object.values(branch?.parsed || {}).map((v) => Number(v)).filter(Number.isFinite);
    if (!parsedValues.length) return null;
    return Math.round((parsedValues.reduce((sum, value) => sum + value, 0) / parsedValues.length) * 100);
  }
  ...
  function renderConsole(logs) {
    ...
    mount.innerHTML = items.map((entry) => {
      ...
      return `<div class="console-line">${escapeHtml(`[${stamp}] ${message}`)}</div>`;
    }).join('');
  }
  ...
  function appendConsole(log) {
    ...
    mount.innerHTML += `<div class="console-line">${escapeHtml(`[${timestamp}] `)}${message}</div>`;
  ...
  function renderAnalysisShell(result = {}, rows = [], version = SYSTEM_VERSION) {
    ...
    if (footerMount) {
      const overallAuditorScore = result?.overallAuditorScore ?? result?.overall_score ?? result?.overallScore ?? result?.auditScore ?? result?.score ?? null;
      const batchGrade = result?.batchGrade ?? result?.batch_grade ?? result?.grade ?? result?.finalGrade ?? null;
      const footerItems = [];
      if (overallAuditorScore !== null && overallAuditorScore !== undefined && String(overallAuditorScore).trim() !== '') footerItems.push(`<div class="audit-grade-box"><div class="label">Overall Auditor Score</div><div class="value">${escapeHtml(String(overallAuditorScore))}</div></div>`);
      if (batchGrade !== null && batchGrade !== undefined && String(batchGrade).trim() !== '') footerItems.push(`<div class="audit-grade-box"><div class="label">Batch Grade</div><div class="value">${escapeHtml(String(batchGrade))}</div></div>`);
      if (footerItems.length) { footerMount.innerHTML = footerItems.join(''); if (footerCard) footerCard.classList.remove('hidden'); }
      else { footerMount.innerHTML = ''; if (footerCard) footerCard.classList.add('hidden'); }
    }
