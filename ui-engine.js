...
(() => {
  ...
  function renderAnalysisShell(result = {}, rows = [], version = SYSTEM_VERSION) {
    ...
    if (summary) {
      summary.innerHTML = [`<div class="pill">Rows: ${rows.length}</div>`,`<div class="pill">Reliable: ${escapeHtml(String(reliableLegs))}/${escapeHtml(String(rows.length))}</div>`,`<div class="pill ${statusMeta.tone}">Status: ${escapeHtml(statusMeta.label)}</div>`,`<div class="pill">Transport: ${escapeHtml(runStatus === 'FAILED_CONNECTOR' ? 'FAIL' : (runStatus === 'LOADING' ? 'WAIT' : 'OK'))}</div>`,`<div class="pill">Payload: ${escapeHtml(runStatus === 'FAILED_PAYLOAD' ? 'FAIL' : (runStatus === 'LOADING' ? 'WAIT' : 'OK'))}</div>`,`<div class="pill">Integrity: ${escapeHtml(runStatus === 'FAILED_INTEGRITY' ? 'FAIL' : ((runStatus === 'PARTIAL_DECODE' || runStatus === 'WEAK_SIGNAL') ? 'TAIL-AUDIT' : (runStatus === 'INSUFFICIENT_EVIDENCE' ? 'SPARSE' : (runStatus === 'LOADING' ? 'WAIT' : (allReliable ? 'PASS' : 'PENDING')))))}</div>`].join('');
    }
