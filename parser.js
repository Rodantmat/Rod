window.PickCalcParser = (() => {
  ...
  const STAT_BOUND_ALIAS_RX = /\b(?:Pitcher Strikeouts|Strikeouts|Pitching Outs|Pitcher Fantasy Score|Pitcher FS|PFS|Pitches Thrown|Pitch Count|Walks Allowed|Hits Allowed|Earned Runs Allowed|Earned Runs|ER|Hitter Fantasy Score|Hitter FS|HFS|Fantasy Score|Hits\+Runs\+RBIs|Hits\+Runs\+RBI|H\+R\+R\+?BI?S?|HRR|Total Bases|TB|Hits|Runs|RBIs|RBI|Home Runs?|HR|Singles|Doubles|Triples|Walks|Stolen Bases|SB|Hitter Strikeouts|Hitter Ks|Ks|K's|PO|Outs|BB|HA)\b/i;
  ...
  const MLB_PROP_ALIASES = {
    ...
    'SB': { label: 'Stolen Bases', key: 'stolenBases', role: 'Batter' },
    ...
    'BATTER STRIKEOUTS': { label: 'Hitter Strikeouts', key: 'hitterStrikeouts', role: 'Batter' },
    'PITCHES THROWN': { label: 'Pitches Thrown', key: 'pitchesThrown', role: 'Pitcher' },
    'PITCH COUNT': { label: 'Pitches Thrown', key: 'pitchesThrown', role: 'Pitcher' },
    'FANTASY SCORE': { label: 'Hitter Fantasy Score', key: 'hitterFantasyScore', role: 'Batter' }
  };

  const MLB_FEED_MATRIX = ['Pitcher Strikeouts','Pitching Outs','Pitcher Fantasy Score','Pitches Thrown','Walks Allowed','Hits Allowed','Earned Runs Allowed','Hitter Fantasy Score','Hits+Runs+RBIs','Total Bases','Hits','Runs','RBIs','Home Runs','Singles','Doubles','Triples','Walks','Stolen Bases','Hitter Strikeouts'];
  ...
  function parseCluster(cluster) {
    ...
    const lines = (cluster || []).map((item) => {
      const raw = typeof item === 'string' ? item : (item?.raw || item?.clean || '');
      return cleanWhitespace(splitGluedTokens(String(raw || '')).replace(DECON, ' ').replace(BADGE_RX, ' '));
    });

    let player = '', team = '', prop = '', line = 0;
    const TEAM_RX = /([A-Z]{2,3})\s*[-—–]\s*(P|SP|RP|C|1B|2B|3B|SS|LF|CF|RF|OF|IF|DH|UTIL)/i;
    const JUNK = /(?:vs\.?|@|Sun|Mon|Tue|Wed|Thu|Fri|Sat|\d{1,2}:\d{2}\s*(?:am|pm)?)/i;

    lines.forEach((l) => { if (/^\d+(?:\.\d+)?$/.test(l)) line = parseFloat(l); });

    for (let i = 0; i < lines.length; i += 1) {
      const current = lines[i];
      if (!TEAM_RX.test(current)) continue;
      const match = current.match(TEAM_RX);
      team = (match?.[1] || '').toUpperCase();
      const hunt = [i - 1, i + 1, i - 2, i + 2, i - 3, i + 3];
      for (const idx of hunt) {
        const rawCandidate = cleanWhitespace(lines[idx] || '');
        if (!rawCandidate) continue;
        const candidate = sanitizePlayerName(rawCandidate);
        if (candidate && candidate.length > 3 && !TEAM_RX.test(rawCandidate) && !JUNK.test(rawCandidate) && !/^\d/.test(rawCandidate) && isLikelyPlayerName(candidate)) {
          player = candidate;
          break;
        }
      }
    ...
    if (!player) {
      for (const raw of lines) {
        const candidate = sanitizePlayerName(raw);
        if (candidate && isLikelyPlayerName(candidate)) { player = candidate; break; }
      }
    }

    const blob = lines.join(' ');
    const directMeta = resolvePropAlias(blob, 'MLB');
    if (directMeta?.label) prop = directMeta.label;
    else if (/Ks|Strikeouts/i.test(blob)) prop = 'Pitcher Strikeouts';
    else if (/Total Bases|TB/i.test(blob)) prop = 'Total Bases';
    else if (/Pitching Outs|Outs|PO/i.test(blob)) prop = 'Pitching Outs';
    else if (/Pitches Thrown|Pitch Count/i.test(blob)) prop = 'Pitches Thrown';
    else if (/Fantasy|HFS|PFS/i.test(blob)) prop = /(?:P|SP|RP)/i.test(blob) ? 'Pitcher Fantasy Score' : 'Hitter Fantasy Score';
