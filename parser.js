window.PickCalcParser = (() => {
  ...
  function resolveProp(clusterLines) {
    const upperLines = clusterLines.map((line) => line.toUpperCase());
    for (const line of upperLines) {
      for (const key of Object.keys(MLB_PROP_ALIASES)) {
        if (line.includes(key)) return MLB_PROP_ALIASES[key];
      }
    }
    return null;
  }
  ...
  function parseCluster(clusterLines, lineValue, idx) {
    ...
    const propMeta = resolveProp(cleaned);
