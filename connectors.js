...
(() => {
  ...
  function inspectPayloadConsistency(data = []) {
    ...
    if (!items.length) return { good: false, reason: 'EMPTY_DATA' };
    let anyGood = false;
    const details = [];
    for (const entry of items) {
      const vals = Array.isArray(entry?.v) ? entry.v.map((v) => safeNumber(v, 0)) : [];
      if (vals.length !== 72) {
        details.push({ good: false, reason: 'BAD_LENGTH', coreNonZero: 0, variance: 0, uniqueCount: 0 });
        continue;
      }
      ...
      const collapsed = coreNonZero < 12;
      const flat = variance < 0.04 || uniqueCount < 15;
      const good = !collapsed && !flat;
      if (good) anyGood = true;
      details.push({ good, reason: collapsed ? 'COLLAPSED_PAYLOAD' : (flat ? 'LOW_VARIANCE' : 'GOOD'), coreNonZero, variance, uniqueCount });
    }
    return { good: anyGood, reason: anyGood ? 'GOOD' : (details[0]?.reason || 'INVALID'), details };
  }
  ...
  async function fetchGeminiBatch(batch) {
    ...
    const basePrompt = `You are an elite sharp analyst. Generate weighted floats (0.0 to 1.0) based on 2026 Statcast and environmental data. High Air Density must penalize Power; Wide Umpire Zones must boost Strikeouts. 0.5 is the fail-state.
...
Return only valid JSON with shape {"data":[{"i":0,"v":[72 floats]}]}.`;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      const body = JSON.stringify({ 
        ...
        generationConfig: { responseMimeType: "application/json", temperature: attempt === 1 ? 0.15 : 0.45 }
      });
