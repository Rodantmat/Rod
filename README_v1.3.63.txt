...
v1.3.63 - RBI Under Market Bonus Tie-Breaker
...
2. Open Control Room.
3. Confirm the visible version tag says v1.3.63 - RBI Under Market Bonus Tie-Breaker.
...
v1.3.63 change:
- Adds stored RBI UNDER market-presence signal as a small scoring tiebreaker bonus in the full scoring refresh.
- Uses Sleeper certified under signals and optional future BettingPros/Gemini prompt output table if present.
- Does not call Gemini or external APIs inside scoring.
- Keeps RBI fallback score cap at 85 and treats Odds API as supplemental for RBI.
