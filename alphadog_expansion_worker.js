const SYSTEM_VERSION = 'v0.1.15 - Phase 1 Score Audit Lens';
...
<div class="top"><h1>AlphaDog Expansion Control Room</h1><div class="version">v0.1.15 - Phase 1 Score Audit Lens</div><div class="small" id="originStatus">Starting UI...</div><div class="status" id="actionStatus">Ready.</div></div>
...
async function applySchema(env) {
  ...
  return { ok:true, version:SYSTEM_VERSION, function_name:'handleApplySchema', route:'/xp/schema/apply', schema:'xp_* isolated schema applied', statement_runner:'individual_prepare_run_no_exec_multistatement', statements_applied:applied.length };
}
...
async function buildPhase1Context(env, options = {}) {
  const runId = makeId('xp_context'); const expansionPhase = Number(options.expansion_phase || 1);
  await applySchema(env); await env.DB.prepare(`INSERT INTO xp_job_runs(run_id, job_name, status, source_table) VALUES (?, 'build_xp_phase1_context_current', 'RUNNING', 'xp_game_bridge_current')`).bind(runId).run();
  try {
    const delM = await env.DB.prepare(`DELETE FROM xp_phase1_player_metrics_current`).run();
    const delC = await env.DB.prepare(`DELETE FROM xp_phase1_game_context_current`).run();
    const lines = await env.DB.prepare(`SELECT x.*, b.game_id, b.away_team, b.home_team, b.normalized_team, b.normalized_opponent, b.match_status AS bridge_match_status FROM xp_prop_lines_current x LEFT JOIN xp_game_bridge_current b ON b.xp_line_key=x.xp_line_key WHERE x.expansion_phase=? AND x.stat_type IN ('Hitter Strikeouts','Walks')`).bind(expansionPhase).all();
    let metricWritten = 0, contextWritten = 0;
    for (const x of (lines.results || [])) {
      const m = await env.DB.prepare(`SELECT * FROM incremental_player_metrics WHERE lower(replace(replace(replace(trim(COALESCE(player_name,'')), '.', ''), '''', ''), ' ', '')) = ? AND UPPER(COALESCE(team_id,'')) IN (?, ?) ORDER BY updated_at DESC LIMIT 1`).bind(x.normalized_player_name, x.team, x.normalized_team || x.team).first();
      const metricStatus = m ? 'MATCHED_NAME_TEAM' : 'MISSING_METRICS';
      const seasonK = m && Number(m.total_pa || 0) > 0 ? Number(m.total_strikeouts || 0) / Number(m.total_pa || 1) : null;
      const seasonBb = m && Number(m.total_pa || 0) > 0 ? Number(m.total_walks || 0) / Number(m.total_pa || 1) : null;
      await env.DB.prepare(`INSERT OR REPLACE INTO xp_phase1_player_metrics_current(xp_line_key, stat_type, player_name, normalized_player_name, team, normalized_team, opponent, line_score, odds_type, player_id, metric_team, role, season, games_logged, first_game_date, last_game_date, total_pa, total_ab, total_hits, total_rbi, total_home_runs, total_walks, total_strikeouts, last3_games, last3_hits, last3_ab, last5_games, last5_hits, last5_ab, last10_games, last10_hits, last10_ab, last20_games, last20_hits, last20_ab, season_k_rate, season_bb_rate, source_confidence, metric_match_status, warning) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(x.xp_line_key, x.stat_type, x.player_name, x.normalized_player_name, x.team, x.normalized_team, x.opponent, x.line_score, x.odds_type, m?.player_id || null, m?.team_id || null, m?.role || null, m?.season || null, m?.games_logged || null, m?.first_game_date || null, m?.last_game_date || null, m?.total_pa || null, m?.total_ab || null, m?.total_hits || null, m?.total_rbi || null, m?.total_home_runs || null, m?.total_walks || null, m?.total_strikeouts || null, m?.last3_games || null, m?.last3_hits || null, m?.last3_ab || null, m?.last5_games || null, m?.last5_hits || null, m?.last5_ab || null, m?.last10_games || null, m?.last10_hits || null, m?.last10_ab || null, m?.last20_games || null, m?.last20_hits || null, m?.last20_ab || null, seasonK, seasonBb, m?.source_confidence || null, metricStatus, m ? null : 'MISSING_METRICS').run();
      metricWritten++;
      const g = x.game_id ? await env.DB.prepare(`SELECT * FROM games WHERE game_id=? LIMIT 1`).bind(x.game_id).first() : null;
      const l = x.game_id ? await env.DB.prepare(`SELECT * FROM lineups_current WHERE game_id=? AND UPPER(team_id)=? AND lower(replace(replace(replace(trim(COALESCE(player_name,'')), '.', ''), '''', ''), ' ', ''))=? ORDER BY is_confirmed DESC, slot ASC LIMIT 1`).bind(x.game_id, x.normalized_team || x.team, x.normalized_player_name).first() : null;
      const s = x.game_id ? await env.DB.prepare(`SELECT * FROM starters_current WHERE game_id=? AND UPPER(team_id)=? ORDER BY confidence DESC, updated_at DESC LIMIT 1`).bind(x.game_id, x.normalized_opponent || x.opponent).first() : null;
      const warning = [];
      if (!m) warning.push('MISSING_METRICS');
      if (!l) warning.push('MISSING_LINEUP');
      if (!s) warning.push('MISSING_STARTER');
      if (String(x.odds_type || 'standard').toLowerCase() !== 'standard') warning.push('NON_STANDARD_LINE_TYPE');
      const contextStatus = m ? 'READY_CONTEXT' : 'WARNING_MISSING_METRICS';
      await env.DB.prepare(`INSERT OR REPLACE INTO xp_phase1_game_context_current(xp_line_key, stat_type, player_name, normalized_player_name, team, opponent, normalized_team, normalized_opponent, line_score, odds_type, start_time, slate_date, game_id, away_team, home_team, venue, game_status, lineup_slot, lineup_bats, lineup_k_rate, lineup_is_confirmed, lineup_match_status, starter_name, starter_throws, starter_era, starter_whip, starter_strikeouts, starter_walks, starter_innings_pitched, starter_match_status, player_id, total_pa, total_walks, total_strikeouts, season_k_rate, season_bb_rate, bridge_match_status, metric_match_status, context_status, warning_flags) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(x.xp_line_key, x.stat_type, x.player_name, x.normalized_player_name, x.team, x.opponent, x.normalized_team, x.normalized_opponent, x.line_score, x.odds_type, x.start_time, x.slate_date, x.game_id || null, x.away_team || null, x.home_team || null, g?.venue || null, g?.status || null, l?.slot || null, l?.bats || null, l?.k_rate || null, l?.is_confirmed ?? null, l ? 'MATCHED_LINEUP' : 'MISSING_LINEUP', s?.starter_name || null, s?.throws || null, s?.era || null, s?.whip || null, s?.strikeouts || null, s?.walks || null, s?.innings_pitched || null, s ? 'MATCHED_STARTER' : 'MISSING_STARTER', m?.player_id || null, m?.total_pa || null, m?.total_walks || null, m?.total_strikeouts || null, seasonK, seasonBb, x.bridge_match_status || 'MISSING_BRIDGE', metricStatus, contextStatus, warning.length ? warning.join('|') : null).run();
      contextWritten++;
    }
    ...
    await env.DB.prepare(`UPDATE xp_job_runs SET status='COMPLETED', rows_read=?, rows_written=?, rows_deleted=?, completed_at=CURRENT_TIMESTAMP WHERE run_id=?`).bind((lines.results || []).length, contextWritten, (delM.meta?.changes || 0) + (delC.meta?.changes || 0), runId).run();
    return { ok:true, run_id:runId, version:SYSTEM_VERSION, expansion_phase:expansionPhase, deleted_metric_rows:delM.meta?.changes || 0, deleted_context_rows:delC.meta?.changes || 0, rows_read:(lines.results || []).length, metric_rows_written:metricWritten, context_rows_written:contextWritten, metric_counts:metricCounts.results || [], context_counts:contextCounts.rows || [] };
  } catch (err) { return { ok:false, run_id:runId, version:SYSTEM_VERSION, error:err?.message || String(err) }; }
}
...
async function buildPhase1Score(env, options = {}) {
  const runId = makeId('xp_score'); await applySchema(env); await env.DB.prepare(`INSERT INTO xp_job_runs(run_id, job_name, status, source_table) VALUES (?, 'build_xp_phase1_score_current', 'RUNNING', 'xp_phase1_game_context_current')`).bind(runId).run();
  try {
    const del = await env.DB.prepare(`DELETE FROM xp_phase1_score_current`).run();
...
export default {
  async fetch(request, env) {
    ...
    if (path === '/health' || path === '/xp/health') return jsonResponse({ ok:true, version:SYSTEM_VERSION, worker:'alphadog-expansion-v001', route:path, function_name:'handleHealth', mode:'isolated_xp_tables_only', source_read_table:SOURCE_TABLE, writes_allowed_only_to:'xp_* tables', admin_secret_configured:Boolean(env.EXPANSION_ADMIN_TOKEN || env.INGEST_TOKEN), embedded_control_room_token_enabled:true, control_room_served_by_worker:true, phase1_audit_lens:true });
