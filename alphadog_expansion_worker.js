const SYSTEM_VERSION = 'v0.1.16 - Phase 1 Context Table Reset Fix';
...
<div class="top"><h1>AlphaDog Expansion Control Room</h1><div class="version">v0.1.16 - Phase 1 Context Table Reset Fix</div><div class="small" id="originStatus">Starting UI...</div><div class="status" id="actionStatus">Ready.</div></div>
...
async function buildPhase1Context(env, options = {}) {
  ...
  await applySchema(env); await resetPhase1ContextTables(env); await env.DB.prepare(`INSERT INTO xp_job_runs(run_id, job_name, status, source_table) VALUES (?, 'build_xp_phase1_context_current', 'RUNNING', 'xp_game_bridge_current')`).bind(runId).run();
  try {
    const delM = { meta: { changes: 0 } };
    const delC = { meta: { changes: 0 } };
