const SYSTEM_VERSION = 'v0.1.31 - Event Delegation Button Fix';
...
(function(){
'use strict';
var VERSION='v0.1.31 - Event Delegation Button Fix';
...
function selfTest(){ show({ok:true,version:VERSION,button_wiring:'PASS_EVENT_DELEGATION_ACTIVE',handler:'document_click_listener_plus_touchstart_marker',api_base:apiBase(),host:location.hostname,protocol:location.protocol,next_test:'Tap Health. It should return HTTP 200.'}); setStatus('Button wiring self-test passed.'); jumpOutput(); }
...
document.addEventListener('click',function(ev){ var btn=ev.target && ev.target.closest ? ev.target.closest('[data-xp-action]') : null; if(!btn){ return; } ev.preventDefault(); ev.stopPropagation(); setStatus('Tap received: '+btn.getAttribute('data-xp-action')); handleAction(btn.getAttribute('data-xp-action')); },true);
document.addEventListener('touchstart',function(ev){ var btn=ev.target && ev.target.closest ? ev.target.closest('[data-xp-action]') : null; if(btn){ btn.setAttribute('data-touch-seen','1'); } },{passive:true,capture:true});
...
window.XP={run:run,selfTest:selfTest,runManualSql:runManualSql,sampleProp:sampleProp,clearSql:clearSql,selectSql:selectSql,copyOutput:copyOutput,handleAction:handleAction};
...
if(bridgeStatus){ bridgeStatus.textContent='API Base: '+apiBase()+' | Handler: event delegation active'; }
setStatus('Ready. Event delegation loaded.');
show('Ready. v0.1.31 event delegation loaded.');
})();
</script></body></html>`;
