const SYSTEM_VERSION = 'v0.1.32 - UI Token Route Hardening';
...
<div class="top"><h1>AlphaDog Expansion Control Room</h1><div class="version">v0.1.32 - UI Token Route Hardening</div><div class="small" id="originStatus">Starting UI...</div><div class="status" id="actionStatus">Ready.</div></div>
...
<section class="section" id="outputSection"><pre id="output">Ready.</pre><button type="button" class="full" data-xp-action="copy-output">COPY OUTPUT</button></section>
</main><script>
(function(){
'use strict';
var VERSION='v0.1.32 - UI Token Route Hardening';
...
'schema':['/xp/schema/apply','POST',{}],
'refresh-board':['/xp/board/refresh','POST',{include_started:false}],
'board-counts':['/xp/board/counts','GET'],
'jobs':['/xp/jobs','GET'],
'logs':['/xp/logs','GET'],
...
'score-build':['/xp/score/build','POST',{expansion_phase:1}],
'score-counts':['/xp/score/counts','GET'],
'score-audit':['/xp/score/audit?limit=100','GET'],
'score-sample':['/xp/score/sample?limit=100','GET'],
'context-sample':['/xp/context/sample?limit=100','GET'],
'phase2-readiness':['/xp/phase2/readiness-gate','POST',{expansion_phase:2,time_tolerance_minutes:15,include_started:false}],
'phase2-hr-score':['/xp/phase2/home-runs/score-scaffold','POST',{expansion_phase:2,time_tolerance_minutes:15,include_started:false,audit_limit:50}],
...
async function run(path, method, body){ var label=actionLabel(path); setStatus('Running '+label); show({action:label.split(' / ')[0],function_name:label.split(' / ')[1],route:path,method:method,version:VERSION,worker_base:apiBase(),status:'RUNNING'}); jumpOutput(); try{ var opts={method:method || 'GET',cache:'no-store'}; if((method || 'GET') === 'POST'){ opts.headers={'Content-Type':'text/plain;charset=UTF-8'}; opts.body=JSON.stringify(body || {}); } var res=await fetch(apiUrl(path),opts); var text=await res.text(); var response; try{ response=JSON.parse(text); }catch(e){ response=safeText(text); } show({action:label.split(' / ')[0],function_name:label.split(' / ')[1],route:path,http_status:res.status,version:VERSION,worker_base:apiBase(),response:response}); setStatus('Finished '+label+' • HTTP '+res.status); jumpOutput(); }catch(err){ show({ok:false,action:label.split(' / ')[0],function_name:label.split(' / ')[1],route:path,version:VERSION,worker_base:apiBase(),error:String(err&&err.message?err.message:err)}); setStatus('Failed '+label); jumpOutput(); } }
function selfTest(){ show({ok:true,version:VERSION,button_wiring:'PASS_DIRECT_BINDING_AND_DELEGATED_FALLBACK',handler:'direct_onclick_plus_touchend_plus_delegated_fallback',api_base:apiBase(),host:location.hostname,protocol:location.protocol,next_test:'Tap Health. It should return HTTP 200.'}); setStatus('Button wiring self-test passed.'); jumpOutput(); }
...
function handleAction(name){ var def=ACTIONS[name]; if(!def){ show({ok:false,version:VERSION,error:'UNKNOWN_UI_ACTION',action:name}); return; } var mode=def[0]; if(mode==='SELF') return selfTest(); if(mode==='MANUAL_SQL') return runManualSql(); if(mode==='CLEAR_SQL') return clearSql(); if(mode==='SELECT_SQL') return selectSql(); if(mode==='SAMPLE_PROP') return sampleProp(); if(mode==='COPY_OUTPUT') return copyOutput(); return run(def[0],def[1],def[2]); }
var lastActionStamp=0;
var lastActionName='';
function fireButtonAction(ev,btn){
 if(ev && ev.cancelable){ ev.preventDefault(); }
 if(ev && ev.stopPropagation){ ev.stopPropagation(); }
 var name=btn && btn.getAttribute ? btn.getAttribute('data-xp-action') : '';
 if(!name){ return false; }
 var now=Date.now();
 if(name===lastActionName && now-lastActionStamp<650){ return false; }
 lastActionName=name; lastActionStamp=now;
 setStatus('Tap received: '+name);
 handleAction(name);
 return false;
}
function bindButtons(){
 var buttons=document.querySelectorAll('[data-xp-action]');
 for(var i=0;i<buttons.length;i++){
  (function(btn){
   btn.onclick=function(ev){ return fireButtonAction(ev,btn); };
   btn.ontouchend=function(ev){ return fireButtonAction(ev,btn); };
   if(window.PointerEvent){ btn.onpointerup=function(ev){ return fireButtonAction(ev,btn); }; }
  })(buttons[i]);
 }
 if(bridgeStatus){ bridgeStatus.textContent='API Base: '+apiBase()+' | Buttons bound: '+buttons.length+' | Token mode: query string hard-locked'; }
}
document.addEventListener('click',function(ev){ var target=ev.target || null; var btn=target && target.closest ? target.closest('[data-xp-action]') : null; if(btn){ return fireButtonAction(ev,btn); } },false);
document.addEventListener('touchend',function(ev){ var target=ev.target || null; var btn=target && target.closest ? target.closest('[data-xp-action]') : null; if(btn){ return fireButtonAction(ev,btn); } },{passive:false,capture:false});
window.onerror=function(message,source,line,column,error){ show({ok:false,version:VERSION,ui_error:String(message),source:source,line:line,column:column,error:String(error&&error.stack?error.stack:error||'')}); setStatus('UI JavaScript error captured.'); };
window.XP={run:run,selfTest:selfTest,runManualSql:runManualSql,sampleProp:sampleProp,clearSql:clearSql,selectSql:selectSql,copyOutput:copyOutput,handleAction:handleAction,bindButtons:bindButtons,apiUrl:apiUrl,apiBase:apiBase};
bindButtons();
...
show('Ready. v0.1.32 UI token route hardening loaded. Buttons are directly bound plus delegated fallback.');
})();
</script></body></html>`;
...
function corsHeaders(extra = {}) { return { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET,POST,OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type,Authorization,X-XP-Token', ...extra }; }
...
function requireAdmin(request, env) {
  ...
  const token = url.searchParams.get('xp_token') || url.searchParams.get('token') || url.searchParams.get('admin_token') || request.headers.get('x-xp-token') || request.headers.get('authorization')?.replace(/^Bearer\s+/i, '') || '';
  const accepted = [env.EXPANSION_ADMIN_TOKEN, env.XP_ADMIN_TOKEN, env.ADMIN_TOKEN, env.INGEST_TOKEN, EMBEDDED_EXPANSION_ADMIN_TOKEN].filter(Boolean);
  if (accepted.includes(token)) return { ok: true, warning: token === EMBEDDED_EXPANSION_ADMIN_TOKEN ? 'embedded_control_room_token_used' : null };
