const SYSTEM_VERSION = 'v0.1.30 - Button Wiring Hard Lock';
...
<div class="top"><h1>AlphaDog Expansion Control Room</h1><div class="version">v0.1.30 - Button Wiring Hard Lock</div><div class="small" id="originStatus">Starting UI...</div><div class="status" id="actionStatus">Ready.</div></div>
<section class="section"><p class="desc">Isolated expansion room. Reads prepared production data, writes only xp_* tables. Production scoring and Main UI stay untouched.</p><div class="pill" id="bridgeStatus">Worker bridge loading...</div><div class="button-grid"><button type="button" onclick="XP.selfTest()">Button Self Test</button><button type="button" onclick="XP.run('/xp/health','GET')">Health</button><button type="button" class="gold" onclick="XP.run('/xp/schema/apply','POST')">Apply Schema</button><button type="button" onclick="XP.run('/xp/board/refresh','POST',{include_started:false})">Refresh Board</button><button type="button" onclick="XP.run('/xp/board/counts','GET')">Board Counts</button><button type="button" onclick="XP.run('/xp/jobs','GET')">Jobs</button><button type="button" onclick="XP.run('/xp/logs','GET')">Logs</button></div></section>
<section class="section"><h2>Phase 1 Pipeline</h2><p class="small">Use RUN PHASE 1 FULL PIPELINE for normal work. It auto-applies schema, refreshes board, builds bridge, builds context, builds score, and returns audit flags.</p><button type="button" class="full green" onclick="XP.run('/xp/phase1/run-all','POST',{expansion_phase:1,time_tolerance_minutes:15,include_started:false,audit_limit:50})">RUN PHASE 1 FULL PIPELINE</button><div class="button-grid"><button type="button" class="teal" onclick="XP.run('/xp/bridge/build','POST',{expansion_phase:1,time_tolerance_minutes:15})">Build Bridge</button><button type="button" onclick="XP.run('/xp/bridge/counts','GET')">Bridge Counts</button><button type="button" onclick="XP.run('/xp/bridge/unmatched','GET')">Unmatched</button><button type="button" class="teal" onclick="XP.run('/xp/context/build','POST',{expansion_phase:1})">Build Context</button><button type="button" onclick="XP.run('/xp/context/counts','GET')">Context Counts</button><button type="button" onclick="XP.run('/xp/context/completeness','GET')">Completeness</button><button type="button" class="green" onclick="XP.run('/xp/score/build','POST',{expansion_phase:1})">Build Score</button><button type="button" onclick="XP.run('/xp/score/counts','GET')">Score Counts</button><button type="button" class="gold" onclick="XP.run('/xp/score/audit?limit=100','GET')">Score Audit</button><button type="button" onclick="XP.run('/xp/score/sample?limit=100','GET')">Score Sample</button><button type="button" onclick="XP.run('/xp/context/sample?limit=100','GET')">Context Sample</button></div></section>
<section class="section"><h2>Phase 2 Gate</h2><p class="small">Readiness gate for Singles, Doubles, and Home Runs. Home Runs can run a basic internal score scaffold. Singles and Doubles stay blocked until native component metrics exist.</p><button type="button" class="full gold" onclick="XP.run('/xp/phase2/readiness-gate','POST',{expansion_phase:2,time_tolerance_minutes:15,include_started:false})">RUN PHASE 2 READINESS GATE</button><button type="button" class="full green" onclick="XP.run('/xp/phase2/home-runs/score-scaffold','POST',{expansion_phase:2,time_tolerance_minutes:15,include_started:false,audit_limit:50})">RUN PHASE 2 HOME RUN SCORE SCAFFOLD</button></section>
<section class="section"><h2>Sample Prop</h2><div class="sample-row"><select id="sampleProp"><option>Hitter Strikeouts</option><option>Walks</option><option>Singles</option><option>Doubles</option><option>Home Runs</option><option>Runs</option><option>Hits+Runs+RBIs</option><option>Hitter Fantasy Score</option><option>Triples</option><option>Stolen Bases</option><option>Hits</option><option>Total Bases</option><option>RBIs</option></select><button type="button" onclick="XP.sampleProp()">Load Sample</button></div></section>
<section class="section"><h2>Manual SQL</h2><p class="small">Read-only guard active. Supports SELECT, WITH, and PRAGMA. Up to 500 returned rows and larger output copy buffer.</p><textarea id="manualSql">SELECT stat_type, odds_type, score_status, internal_grade, COUNT(*) AS rows_count, MIN(internal_score_0_100) AS min_score, MAX(internal_score_0_100) AS max_score, AVG(internal_score_0_100) AS avg_score FROM xp_phase1_score_current GROUP BY stat_type, odds_type, score_status, internal_grade ORDER BY stat_type, odds_type, max_score DESC LIMIT 100</textarea><div class="sql-buttons"><button type="button" class="gold" onclick="XP.runManualSql()">Run SQL</button><button type="button" onclick="XP.clearSql()">Clear SQL</button><button type="button" onclick="XP.selectSql()">Select SQL</button></div></section>
<section class="section" id="outputSection"><pre id="output">Ready.</pre><button type="button" class="full" onclick="XP.copyOutput()">COPY OUTPUT</button></section>
</main><script>
(function(){
'use strict';
var VERSION='v0.1.30 - Button Wiring Hard Lock';
...
function setStatus(msg){ if(actionStatus){ actionStatus.textContent=msg; } }
function isWorkerHost(){ return /workers\.dev$/i.test(location.hostname); }
function isLocalFile(){ return location.protocol === 'file:'; }
function apiBase(){ return (isWorkerHost() && location.hostname.indexOf('alphadog-expansion-v001') >= 0) ? location.origin : HARD_CODED_WORKER_BASE; }
...
var ACTION_META={
'/xp/health':'Health / handleHealth','/xp/schema/apply':'Apply Schema / handleApplySchema','/xp/board/refresh':'Refresh Board / handleRefreshBoard','/xp/board/counts':'Board Counts / handleCounts','/xp/jobs':'Jobs / handleJobs','/xp/logs':'Logs / handleLogs','/xp/manual-sql':'Manual SQL / handleManualSql','/xp/phase1/run-all':'Run Phase 1 Full Pipeline / handlePhase1RunAll','/xp/phase2/readiness-gate':'Run Phase 2 Readiness Gate / handlePhase2ReadinessGate','/xp/phase2/home-runs/score-scaffold':'Run Phase 2 Home Run Score Scaffold / handlePhase2HomeRunScoreScaffold','/xp/bridge/build':'Build Bridge / handleBuildGameBridge','/xp/bridge/counts':'Bridge Counts / handleBridgeCounts','/xp/bridge/unmatched':'Unmatched / handleBridgeUnmatched','/xp/context/build':'Build Context / handleBuildPhase1Context','/xp/context/counts':'Context Counts / handleContextCounts','/xp/context/completeness':'Completeness / handleContextCompleteness','/xp/context/sample':'Context Sample / handleContextSample','/xp/score/build':'Build Score / handleBuildPhase1Score','/xp/score/counts':'Score Counts / handleScoreCounts','/xp/score/sample':'Score Sample / handleScoreSample','/xp/score/audit':'Score Audit / handleScoreAudit'};
function actionLabel(path){ var clean=String(path||'').split('?')[0]; return ACTION_META[clean] || 'Load Sample / handleSampleProp'; }
function jumpOutput(){ setTimeout(function(){ if(outputSection){ outputSection.scrollIntoView({behavior:'smooth',block:'end'}); } },80); }
function safeText(value){ var s=String(value==null?'':value); if(/^\s*</.test(s)){ return JSON.stringify({ok:false,connection_issue:'HTML_RESPONSE_NOT_JSON',http_preview:s.replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim().slice(0,900),worker_base:apiBase()},null,2); } return s.length>OUTPUT_CHAR_LIMIT ? s.slice(0,OUTPUT_CHAR_LIMIT)+'\n...[truncated by control room output lens at '+OUTPUT_CHAR_LIMIT+' chars]' : s; }
function show(data){ if(!output){ return; } output.textContent = typeof data === 'string' ? safeText(data) : safeText(JSON.stringify(data,null,2)); }
async function run(path, method, body){ var label=actionLabel(path); setStatus('Running '+label); show({action:label.split(' / ')[0],function_name:label.split(' / ')[1],route:path,method:method,version:VERSION,worker_base:apiBase(),status:'RUNNING'}); jumpOutput(); try{ var opts={method:method || 'GET'}; if((method || 'GET') === 'POST'){ opts.headers={'Content-Type':'text/plain;charset=UTF-8'}; opts.body=JSON.stringify(body || {}); } var res=await fetch(apiUrl(path),opts); var text=await res.text(); var response; try{ response=JSON.parse(text); }catch(e){ response=safeText(text); } show({action:label.split(' / ')[0],function_name:label.split(' / ')[1],route:path,http_status:res.status,version:VERSION,worker_base:apiBase(),response:response}); setStatus('Finished '+label+' • HTTP '+res.status); jumpOutput(); }catch(err){ show({ok:false,action:label.split(' / ')[0],function_name:label.split(' / ')[1],route:path,version:VERSION,worker_base:apiBase(),error:String(err&&err.message?err.message:err)}); setStatus('Failed '+label); jumpOutput(); } }
function selfTest(){ show({ok:true,version:VERSION,button_wiring:'PASS_INLINE_ONCLICK_ACTIVE',api_base:apiBase(),host:location.hostname,protocol:location.protocol,next_test:'Tap Health. It should return HTTP 200.'}); setStatus('Button wiring self-test passed.'); jumpOutput(); }
function runManualSql(){ var sqlBox=document.getElementById('manualSql'); run('/xp/manual-sql','POST',{sql:sqlBox ? sqlBox.value : '',max_rows:DEFAULT_SQL_MAX_ROWS}); }
function sampleProp(){ var el=document.getElementById('sampleProp'); run('/xp/board/sample?stat_type='+encodeURIComponent(el ? el.value : 'Hitter Strikeouts')+'&limit=100','GET'); }
function clearSql(){ var el=document.getElementById('manualSql'); if(el){ el.value=''; } setStatus('SQL box cleared.'); }
function selectSql(){ var el=document.getElementById('manualSql'); if(el){ el.focus(); el.select(); } setStatus('SQL selected.'); }
async function copyOutput(){ try{ await navigator.clipboard.writeText(output ? output.textContent || '' : ''); setStatus('Output copied.'); }catch(err){ setStatus('Copy failed. Long-press/select output manually.'); } }
window.XP={run:run,selfTest:selfTest,runManualSql:runManualSql,sampleProp:sampleProp,clearSql:clearSql,selectSql:selectSql,copyOutput:copyOutput};
...
if(bridgeStatus){ bridgeStatus.textContent='API Base: '+apiBase(); }
setStatus('Ready.');
show('Ready.');
})();
</script></body></html>`;
